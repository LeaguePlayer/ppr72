...
class Works extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, image, title, text', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'workImages'=>array(self::HAS_MANY, 'WorkImages',
                'work_id'),
		);
	}
	...
	protected function afterSave() {
	    ...
	    if (isset($images) && count($images) > 0) {
	        foreach($images as $k => $img) {
	            ...
	            if($img->saveAs($_SERVER['DOCUMENT_ROOT'] ."/uploads/works/".$imageName)) {
	                $workImage = new WorkImages();
	                $workImage->work_id = $this->getPrimaryKey();
	                $workImage->url = $imageName;
	                $workImage->save();
	            }
	        }
	    }
